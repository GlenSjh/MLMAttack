    ...
    def calc_ppl(self, texts):
        ...
        with torch.no_grad():
            ...
            for i in range(0, input_ids.size(1), self.stride):
                ...
                eval_loss.append(log_likelihood.item())

        return torch.exp(torch.tensor(eval_loss).sum() / end_loc).item()

    def calc_ppl_mine(self, sentences):
        self.ppl_tokenizer.pad_token = self.ppl_tokenizer.eos_token
        with torch.no_grad():
            ppl_list = []
            for sentence in sentences:
                ipt = self.ppl_tokenizer(sentence, return_tensors="pt", verbose=False)
                ipt.input_ids = ipt.input_ids.cuda()
                ipt.attention_mask = ipt.attention_mask.cuda()
                ppl_list.append(torch.exp(
                    self.ppl_model(input_ids=ipt.input_ids, attention_mask=ipt.attention_mask, labels=ipt.input_ids)[0]).item())

        return torch.tensor(ppl_list).mean().item()


    def batch_ppl(self, sentences):
        self.ppl_tokenizer.pad_token = self.ppl_tokenizer.eos_token
        pre_half_sents = sentences[:int(len(sentences)/2)]
        post_half_sents = sentences[int(len(sentences)/2):]
        ppl_list = []
        loss_fct = torch.nn.CrossEntropyLoss()

        for sentence in [pre_half_sents, post_half_sents]:
            if(sentence):
                ipt = self.ppl_tokenizer(sentence, padding=True, return_tensors="pt", verbose=False)
                ipt.input_ids = ipt.input_ids.cuda()
                ipt.attention_mask = ipt.attention_mask.cuda()
                logits = self.ppl_model(input_ids=ipt.input_ids, attention_mask=ipt.attention_mask).logits
                labels = ipt.input_ids
                for i in range(logits.shape[0]):
                    _logits = logits[i][ipt.attention_mask.bool()[i]]
                    _labels = labels[i][ipt.attention_mask.bool()[i]]

                    shift_logits = _logits[..., :-1, :].contiguous()
                    shift_labels = _labels[..., 1:].contiguous()
                    ppl = loss_fct(shift_logits.view(-1, shift_logits.size(-1)), shift_labels.view(-1)).exp()
