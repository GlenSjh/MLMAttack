...
class WordSwapMaskedLM(WordSwap):
    ...
    def _bert_attack_replacement_words(
        ...
        if not len(target_ids_pos):
            ...
            # we did this following TextDefender [https://github.com/RockyLzy/TextDefender/blob/master/textattack/transformations/word_swap_masked_lm.py] since BERT-ATTACK is too slow!
            return []
            ...
            for bpe_tokens in tqdm(products):
                ...
                perplexity = cross_entropy_loss(logits, word_tensor)
                '''
                word = "".join(
                ...
                '''
                combination_results.append((word_tensor.tolist(), perplexity))
            # Sort to get top-K results
            sorted(combination_results, key=lambda x: x[1])
            top_replacements = [
                "".join(
                    self._lm_tokenizer.convert_ids_to_tokens(x[0])
                ).replace("##", "") for x in combination_results[: self.max_candidates]
            ]
